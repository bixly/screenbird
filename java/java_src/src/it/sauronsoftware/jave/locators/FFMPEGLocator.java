...
public abstract class FFMPEGLocator {
    ...
    public FFMPEGExecutor createExecutor() {
        return new FFMPEGExecutor(Settings.getFFMpegExecutable());
    }

    /**
     * Selects and returns the locator
     */
    public static FFMPEGLocator getLocator() {
        ...
        if (isWindows) {
            locator = new WindowsFFMPEGLocator();
        }
        else {
            //linux
            if (System.getProperty("mrj.version") == null) {
                locator = new LinuxFFMPEGLocator();
            }
            else {
                //mac
                locator = new MacFFMPEGLocator();
            }
        }
